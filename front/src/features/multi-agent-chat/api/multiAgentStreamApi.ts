...
export class MultiAgentStreamApi {
  ...
  private getStreamApiUrl(): string {
    ...
    if (backendUrl) {
      return `${backendUrl}/api/${apiVersion}/multi-agent/chat/stream`;
    }

    // 기본값 (개발용)
    return "http://localhost:8000/api/v1/multi-agent/chat/stream";
  }
