...
class NotionMCPClient:
    ...
    @notionhq/notion-mcp-server를 사용하여 MCP 도구를 로드
    ...
    async def initialize(self) -> List[Any]:
        ...
        Raises:
            ValueError: NOTION_API_KEY가 설정되지 않은 경우
        """
        ...
        # 토큰 확인
        token = self.notion_token or os.environ.get("NOTION_API_KEY")
        if not token:
            raise ValueError(
                "NOTION_API_KEY가 필요합니다. "
